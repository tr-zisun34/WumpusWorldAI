...
#         self.arrow = None
#         self.font = pygame.font.Font(FONT_MRSMONSTER, 30)
#         self.noti = pygame.font.Font(FONT_MRSMONSTER, 20)
#         self.victory = pygame.font.Font(FONT_MRSMONSTER, 60)
#         self.all_sprites = pygame.sprite.Group()
#         self.state = MAP
#         self.map_i = 1
#         self.mouse = None
...
#         # Draw simple HUD
#         score = self.agent.get_score()
#         score_text = self.font.render(f'Score: {score:,}', True, (70, 130, 180))  # Steel blue text
...
#         # Draw buttons with dynamic spacing to avoid overlap
#         button_height = 60
#         start_y = 150
#         max_buttons = 6
#         spacing = max((SCREEN_HEIGHT - start_y - 100) // max_buttons, 60)  # Ensure minimum spacing
#         if start_y + (max_buttons - 1) * spacing + button_height > SCREEN_HEIGHT:
#             spacing = (SCREEN_HEIGHT - start_y - button_height) // (max_buttons - 1)
            
#         for i, text in enumerate(["MAP 1", "MAP 2", "MAP 3", "MAP 4", "MAP 5", "EXIT"]):
#             rect = pygame.Rect(285, start_y + i * spacing, 400, button_height)
#             color = (100, 180, 220) if rect.collidepoint(self.mouse) else (135, 206, 235)  # Sky blue shades
#             text_color = (255, 245, 220) if rect.collidepoint(self.mouse) else (70, 130, 180)  # Cream and steel blue
#             self.draw_button(self.screen, rect, color, text_color, text)
...
#             elif event.type == pygame.MOUSEBUTTONDOWN:
#                 self.mouse = pygame.mouse.get_pos()
#                 if 285 <= self.mouse[0] <= 685:
#                     if 150 <= self.mouse[1] <= 210:
#                         self.state = RUNNING
#                         self.map_i = 1
#                     elif 230 <= self.mouse[1] <= 290:
#                         self.state = RUNNING
#                         self.map_i = 2
#                     elif 310 <= self.mouse[1] <= 370:
#                         self.state = RUNNING
#                         self.map_i = 3
#                     elif 390 <= self.mouse[1] <= 450:
#                         self.state = RUNNING
#                         self.map_i = 4
#                     elif 470 <= self.mouse[1] <= 530:
#                         self.state = RUNNING
#                         self.map_i = 5
#                     elif 550 <= self.mouse[1] <= 610:
#                         pygame.quit()
#                         sys.exit()
...
#         color = (255, 245, 220) if self.state == WIN else (70, 130, 180)  # Cream for victory, steel blue for defeat
#         text = self.victory.render(message, True, color)
#         text_rect = text.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//3))
#         self.screen.blit(text, text_rect)
        
#         score = self.agent.get_score()
#         score_text = self.victory.render(f'Score: {score:,}', True, (255, 255, 255))  # White text
...
#                     if action == Algorithms.Action.KILL_ALL_WUMPUS_AND_GRAB_ALL_FOOD:
#                         self.state = WIN
#                     if action in [Algorithms.Action.FALL_INTO_PIT, Algorithms.Action.BE_EATEN_BY_WUMPUS]:
#                         self.state = GAMEOVER
#                         break
...
#             elif self.state in [WIN, TRYBEST]:
#                 self.win_draw()
#                 self.win_event()
...
#         elif action == Algorithms.Action.KILL_WUMPUS:
#             i, j = self.agent.get_pos()
#             if self.direct == 0: i -= 1
#             elif self.direct == 1: i += 1
#             elif self.direct == 2: j -= 1
#             elif self.direct == 3: j += 1
#             self.wumpus.wumpus_killed(i, j)
#             self.wumpus.wumpus_notification()
#             if not self.wumpus.stench_i_j(i, j):
#                 self.wumpus.wumpus_kill(self.screen, self.font)
